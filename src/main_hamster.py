...
parser.add_argument('--output_dir','-o', type=str, default='./results_self_distillation_hamster/', help='Directory to store results')
...
Follow these rules:
0. Only actions to use for plans: "move", "place", "pick up and place", "pick up and put", "put", "hit", "push", "press"
1. Base your plan strictly on what is visible in the image. Do not make assumptions or use “if” statements where visual evidence is available.
2. Use only one physical action per step. Avoid combining actions like "grasp and place" in a single step.
3. Ground each step in spatial or physical reasoning if relevant (e.g., proximity, reachability, obstructions).
4. Each step should be self-contained and atomic—either a perception, decision, or action.
5. Do not speculate about tools or options not seen in the image.
6. Avoid vague directives like “monitor,” “ensure,” or “check” unless grounded in specific sensor input or environmental cue visible in the image.
...
Your goal is to identify critical flaws, gaps, or missed opportunities that would significantly improve the plan’s feasibility, clarity, or alignment with the depicted environment in achieving the user's goal. Focus on major missing steps, unrealistic assumptions, or vague actions that reduce the quality of the plan. Avoid nitpicking or commenting on minor stylistic issues.
...
Focus on criticizing:
- Use of any actions that are not "move", "place", "pick up and place", "pick up and put", "put", "hit", "push", or "press"
- Use of conditional statements where certainty is possible based on the image.
- Multiple actions combined into one step (violates atomicity).
- Missing perception steps or spatial reasoning (e.g., not checking for obstructions or visibility).
- Any vague or speculative instructions not grounded in visual data.
...
Use the feedback to address key flaws with substantive improvements. Focus on clarity, feasibility, and grounding the plan in the actual visual context. Prioritize corrections that enable the robot to effectively and realistically complete the task.
...
Focus on revising the plans by following these rules:
- Only using the following actions for the plans: "move", "place", "pick up and place", "pick up and put", "put", "hit", "push", "press"
- Removing unnecessary conditional logic by inferring from the image.
- Making each step atomic (only one action or decision per step).
- Adding missing perceptual/spatial reasoning where needed (e.g., line of sight, object accessibility).
- Avoiding vague goals like “monitor” or “ensure” unless clearly tied to observable image context.
- Achieve the goal of the user input.
...
def final_question(user_input, initial_plan, revised_plan):
    ...
    instructions = '''You are comparing two sets of plans for a robot in the image.
