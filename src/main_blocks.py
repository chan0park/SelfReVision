...
parser.add_argument('--output_dir','-o', type=str, default='./results_self_distillation_blocks/', help='Directory to store results')
parser.add_argument('--data_path','-d', type=str, default='./validation-data/block_eval.csv', help='Path to input data CSV'),
...
parser.add_argument('--max_refinement_per_round','-mrr', type=int, default=2, help='Maximum refinements per round')
...
def first_prompt(user_input, items):
    instructions = f'''
You are writing instructions for a robot to rearrange blocks into a meaningful shape or pattern based on a goal.

Follow this process:
1. First, describe a high-level **layout plan** that assigns blocks to parts of the shape (e.g., "eyes", "corners", "mouth").
2. Then, write a detailed **movement plan** using exact coordinates to place each block to achieve the layout. 
3. Use only listed items.
4. Use normalized coordinates in the range [0, 1] × [0, 1] (where (0, 0) is bottom-left and (1, 1) is top-right)
5. Not all blocks need to be used. Decide the optimal number of blocks to use to achieve the goal.

Valid action format:
Move [object] to [coordinates] (e.g., "Move the green block to [0.5, 0.4]")
...
Do not include any other text, formatting, or markdown, and the plan has to be a list of numbered steps.

Examples:
Goal: Create a Y with the blocks provided
Items: red block, yellow block, brown block, purple block, green block
Plan: <plan>1. Move the red block to (.4,.1)
2. Move the yellow block to (.4,.3)
3. Move the green block to (.2,.5)
4. Move the brown block to (.6,.5)</plan>

Goal: Create a circle with the blocks provided
Items: pink block, gray block, black block, red block, green block, brown block, blue block, and yellow block.
Plan: <plan>1. Move the red block to (.5,.95)
2. Move the yellow block to (.75,.85)
3. Move the green block to (.9,.5)
4. Move the brown block to (.75,.15)
5. Move the gray block to (.5, .05)
6. Move the blue block to (.25, .15)
7. Move the pink block to (.1, .5)
8. Move the black block to (.25, .85)</plan>
...
def self_criticism_prompt(user_input, current_plan, items):
    """Generate a prompt for the model to critique its own plan."""
    instructions = f'''You are reviewing a robot's block placement plan (coordinates) based on the specified goal and the list of available blocks.
Your task is to identify **major flaws** that would prevent the robot from achieving the specified layout in the goal correctly. Prioritize issues on coordinates and object types. 

Follow these criteria:
1. Consider the layout implied by the goal (e.g., letter shape, smiley face).
2. Check if all necessary components are represented (e.g., two eyes).
3. Ensure block placements (coordinates) are placd to form the desired shape.
4. Watch for duplicate block references or contradictory actions.
5. The plan must only use the listed items and coordinates in the range [0, 1] × [0, 1] (where (0, 0) is bottom-left and (1, 1) is top-right).
6. Not all blocks need to be used. Decide the optimal number of blocks to use to achieve the goal.
7. Valid action format: Move [object] to [coordinates] (e.g., "Move the green block to [0.5, 0.4]")
...
def self_revision_prompt(user_input, current_plan, criticism, items):
    """Generate a prompt for the model to revise its plan based on criticism."""
    instructions = f'''You are revising a robot's block placement plan based on a visual goal and critical feedback.

Revise the plan as follows:
1. Correct the errors identified in the feedback.
2. Ensure all required blocks are placed in appropriate roles to fulfill the layout of the goal.
3. Keep the plan minimal but complete—do not omit essential steps or objects.
4. Omit redundant or overly detailed instructions that don't improve execution.
5. The plan must only use the listed items and coordinates in the range [0, 1] × [0, 1] (where (0, 0) is bottom-left and (1, 1) is top-right).
6. Not all blocks need to be used. Decide the optimal number of blocks to use to achieve the goal.
7. Valid action format: Move [object] to [coordinates] (e.g., "Move the green block to [0.5, 0.4]")
...
Output a clean, single-level numbered list of steps enclosed between <plan> and </plan>.
...
def final_question(user_input, initial_plan, revised_plan, items):
    """Compare two plans based on their ability to spatially fulfill the visual goal with blocks."""
    instructions = f'''You are comparing two robot plans for arranging blocks to fulfill the specified visual goal.

Your decision must be based on how well each plan uses the available blocks to form the correct **spatial layout** for the goal.
Follow these criteria:
1. Consider the layout implied by the goal (e.g., letter shape, smiley face).
2. Check if all necessary components are represented (e.g., two eyes).
3. Ensure block placements (coordinates) are placd to form the desired shape.
4. Avoid duplicate block references or contradictory actions.
5. The plan must only use the listed items and coordinates in the range [0, 1] × [0, 1] (where (0, 0) is bottom-left and (1, 1) is top-right).
6. Valid action format: Move [object] to [coordinates] (e.g., "Move the green block to [0.5, 0.4]")

If Plan 2 creates a **more complete or accurate shape** than Plan 1, respond only with: yes
...
def extract_criticism_from_response(response):
    """Extract the criticism from the model's response."""
    try:
...
def run_self_criticism_and_revision(model, processor, user_input, image_path, items, current_plan, pbar, idx, round_num, sample_results, sample_steps, temperature):
    ...
    while num_refinement < MAX_REFINEMENT:
        ...
        criticism_prompt = self_criticism_prompt(user_input, current_plan, items)
        ...
        revision_prompt = self_revision_prompt(user_input, current_plan, criticism, items)
        ...
        if bool_revision_success:
            prompt = final_question(user_input, best_plan, revised_plan, items)
...
def run_self_distillation():
    ...
    for idx, row in pbar:
        ...
        while not bool_success and num_try < 5:
            initial_prompt = first_prompt(user_input, items)
        ...
        for round_num in range(1, MAX_ROUND + 1):
            ...
            best_round_plan, best_plan_round, num_refinement, num_inference = run_self_criticism_and_revision(
                model, processor, user_input, image_path, items, current_plan, pbar, idx, round_num, sample_results, sample_steps, temperature
            )
            ...
            if num_refinement > 0:  # Only validate if refinements were made
                ...
                prompt_vs_p0 = final_question(user_input, initial_plan, best_round_plan, items)
                ...
                if global_best_plan != initial_plan:
                    prompt_vs_global = final_question(user_input, global_best_plan, best_round_plan, items)
